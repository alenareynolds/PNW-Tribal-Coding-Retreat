...
So far we've just constructed a raster container with no values (try plotting what we have so far) - let's provide values to the cells using the `runif` function to derive random values from thedata:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAASCAYAAABit09LAAAAI0lEQVR4XmNgGAWjAB0ICAj8JwYTpZgok1EUwQBRimAAlyIA2/cjg5a5o9gAAAAASUVORK5CYII= uniform distribution:
